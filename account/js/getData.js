...
function get_history(history_list) {
    ...
    for (h = 0; h < history_bank.length; h++) {
        ...
        //1. 현재 들어온 날짜가 account_list.date 중에 있어 ? 
        // 없어! == > account_list 구조랑 똑같은 객체를 만들어줘, account_list 현재 date, 랑 현재 info 넣어주면 돼 
        ...
        if (numTrue) {
            //이미 만들어진 객체 account_list[n]번째의 list(key)에 현재 반복 실행중에 들어온 history_bank[h] 값 넣기
            ...
            //현재 들어온 데이터의 date 값이 새롭다! == 이전의 날짜와 비교해서 새로운 날짜다. 
            //그럼 새 객체 만들어 주기. ==> account_list[obj_num] 여기서 obj_num은 반복 돌때마다 증가
    ...
    for (let al = 0; al < obj_num; al++) {
        ...
        let total_add = 0; //가격합산을 위한 변수 선언
        
        //각 .day의 li 개수만큼 반복실행
        let list_len = account_list[al].list;
        for (let db = 0; db < list_len.length; db++) {
            const history_name = list_len[db].history; //이름
            const history_price = Number(list_len[db].price); //가격
            const history_income = list_len[db].income; //입금 출금
            const li_elm = document.createElement('li'); //li 생성
            ...
            if (history_income == 'in') {
                //입금일 경우 addClass deposit
                price_elm.classList.add('desposit');
            } else {
                //출금일 경우 addClass withdraw, 가격 더해주기
            ...
            // total_add += history_price;
            name_elm.textContent = history_name; //name 찍어주기
            price_elm.textContent = comma(history_price); //가격 부분 찍어주고, comma 생성
